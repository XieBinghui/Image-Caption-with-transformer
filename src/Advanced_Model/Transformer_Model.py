...
class Transformer(nn.Module):
    ...
    def update_current_word(self, y, true_words, t, cuda):
        ...
        current_word = np.zeros((128,9956))
...
#         print("**************")
    ...
    def beam_search(self, image_features, max_nb_words, beam_width):
        ...
        end_word = self.indexto1hot(len(self.vocab), self.vocab('<end>'))
...
#         # print(np.shape(image_feats))
#         nb_batch, nb_image_feats, _ = image_feats.size()
#         # nb_batch, _ = image_feats.size()
#         use_cuda = image_feats.is_cuda

#         v_mean = image_feats.mean(dim=1)
#         # v_mean = image_feats.unsqueeze(1)

#         state, current_word = self.init_inference(nb_batch, use_cuda, self.vocab)
#         y_out = make_zeros((nb_batch, nb_timesteps-1, self.dict_size),
#                                  cuda = use_cuda)
...
#     def forward_one_step(self, state, current_word, v_mean, image_feats):
#         nb_batch, image_feat_dim, _ = image_feats.size()
#         # word_emb = self.embed_word(current_word).unsqueeze(0)
#         word_emb = self.embed_word(current_word)
#         print("word_emb {}".format(np.shape(word_emb)))
#         print("state {}".format(np.shape(state)))
#         if state is None:
#             state = word_emb
#         else:
#             state = torch.cat((state, word_emb), dim=1)
...
#         current_word = self.indexto1hot(len(self.vocab), next_word)
#         current_word = torch.from_numpy(current_word).float()
...
#     def init_inference(self, nb_batch, cuda, vocab):
#         start_word = self.indexto1hot(len(self.vocab), self.vocab('<start>'))
#         start_word = torch.from_numpy(start_word).float().unsqueeze(0)
#         start_word = start_word.repeat(nb_batch, 1)
