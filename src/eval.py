...
from data_loader_flickr_val import get_loader
import logging
import json
...
class Sentences(object):
    def __init__(self, savedir):
        ...
        if os.path.exists(savedir) == False:
            os.makedirs(savedir)
        self.filepath = os.path.join(savedir, "flickr_lstm.json")

    def add_sentence(self, image_id, sentence):
        caption = ' '.join(sentence[7:-5])
...
#             temp_preds.append(preds[j][:decode_lengths])  # remove pads
        # sampled_ids = temp_preds
        sampled_ids = preds
        # sampled_ids = sampled_ids[0]
        # print(np.shape(sampled_ids)).squeeze(0)
        ...
        s.add_sentence(img_id[0], sentence)
        if (i % 500) == 0:
